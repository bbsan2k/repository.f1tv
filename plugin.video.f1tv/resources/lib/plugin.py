...
def hasMultipleAudioGroups(content):
    """
    Check if there are multiple audio groups in the content
    :param content: m3u8 content
    :return: True if multiple groups are detected
    """
...
def getCorrectedM3U8(stream_url):
    """
    For some playlists it is necessary to fix the playlist before playing, as there are different audio groups
    :param stream_url: URL of the playlist to be fixed
    :return: URL of the playlist to be played
    """
    ...
    if r.ok:
        fix_audio = hasMultipleAudioGroups(r.content)
            ...
            try:
                for line in r.content.splitlines():
                    ...
                    if line.startswith('#EXT-X-MEDIA:') and 'TYPE=CLOSED-CAPTIONS' not in line:
                        ...
                        corrected_uri = "{}{}?{}".format(base_url, uri, params) if 'http' not in uri else "{}?{}".format(uri, params)
                        ...
                        out_line = "{}{}?{}".format(base_url, line, params) if 'http' not in line else "{}?{}".format(line, params)
                    else:
                        out_line = line
...
def playContent(content_url):
    """
    Play content supplied by plugin url
    :param content_url: Url of the content to be played (e.g. channel or asset)
    :return:
    """
    _api_manager.login(_ADDON.getSetting("username"), _ADDON.getSetting("password"))
